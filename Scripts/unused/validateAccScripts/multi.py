...
output_file = 'new_ocr_results.txt'
    ...
    for root, dirs, files in os.walk(test_dir):
        for file in files:
            if file.endswith('.png'):
                ...
                # Check if label file exists and read ground truth
                ground_truth_plate = ""
                ...
                if results and len(results[0].boxes) > 0:
                    ...
                    detected_text = ''.join([result[1].upper().replace(' ', '').replace('|', '1').replace('/', '1') for result in ocr_results])
